...
class Config:
    ...
    buzzer_pin: int = 18  # 连接蜂鸣器的GPIO引脚
    show_window: bool = False  # 是否显示视频窗口
...
def trigger_alert(buzzer_pin: int, alert_duration: int = 2) -> None:
    ...
    Args:
        buzzer_pin (int): 连接蜂鸣器的GPIO引脚。
        alert_duration (int): 哔哔声持续时间（秒）。
    """
    def beep():
        try:
...
def main():
    ...
    GPIO.output(config.buzzer_pin, GPIO.LOW)  # 确保蜂鸣器初始关闭
    logger.info(f"GPIO {config.buzzer_pin} 已初始化为输出。")
    ...
    try:
        while True:
            ...
            for (x1, y1, x2, y2) in plant_boxes:
                ...
                if (disease_detection_count >= config.detection_threshold and
                    ...
                    trigger_alert(config.buzzer_pin, config.alert_duration)
        ...
        cap.release()
        GPIO.output(config.buzzer_pin, GPIO.LOW)  # 确保蜂鸣器关闭
        GPIO.cleanup()
